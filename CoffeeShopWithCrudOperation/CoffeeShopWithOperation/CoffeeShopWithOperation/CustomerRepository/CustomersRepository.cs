...
namespace CoffeeShopWithOperation.CustomerRepository
{
   public class CustomersRepository
    {
        public bool additems(string name, string address,int contract)
        {
            bool isaded = false;
            try
            {
                ...
                string itemsqurey = @" INSERT INTO  Customer (Name,Address,Contract) VALUES('" + name + "','" + address + "',"+contract+")";
        ...
        public DataTable show()
        {
            ...
            string itemsqurey = @" select * from Customer";
            SqlCommand sqlCommand = new SqlCommand(itemsqurey, sqlItems);
            sqlItems.Open();
            SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            DataTable dataTable = new DataTable();
        ...
        public bool update(string name, string address, int contract,int id)
        {
            try
            {
                ...
                string sqlvalues = @"UPDATE Customer SET Name='" + name + "',Address='" + address + "',Contract="+contract+" WHERE ID=" + id + "";
                SqlCommand sqlCommand = new SqlCommand(sqlvalues, sqlConnection);
                sqlConnection.Open();
...

        {
            try
            {
                ...
                string sqlvalues = @"DELETE FROM Customer WHERE ID=" + id + "";
                SqlCommand sqlCommand = new SqlCommand(sqlvalues, sqlConnection);
                sqlConnection.Open();
                int isexcuted = sqlCommand.ExecuteNonQuery();
                if (isexcuted > 0)
